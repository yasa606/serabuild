      ...
      <div className="mx-auto flex h-[76px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="relative flex h-9 w-9 items-end justify-center gap-[3px]">
            {/* Building 1 */}
            <span className="h-5 w-[5px] bg-[#D4A72C] transition-all duration-300 group-hover:h-6" />

            {/* Building 2 */}
            <span className="h-7 w-[5px] bg-[#D4A72C] transition-all duration-300 group-hover:h-8" />

            {/* Building 3 */}
            <span className="h-9 w-[5px] bg-[#D4A72C] transition-all duration-300 group-hover:h-10" />
          ...
          <div className="flex flex-col leading-none">
            ...
            <span className="mt-1 text-[7px] font-bold uppercase tracking-[0.24em] text-[#94A3B8]">
        ...
        <nav className="hidden items-center gap-7 lg:flex">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="relative py-2 text-[11px] font-bold uppercase tracking-[0.12em] text-[#334155] transition-colors duration-200 hover:text-[#D4A72C]"
            >
              {link.name}

              {/* Gold hover line */}
              <span className="absolute bottom-0 left-0 h-[2px] w-0 bg-[#D4A72C] transition-all duration-300 group-hover:w-full" />
            </Link>
          ))}
        </nav>
        ...
        <button
          ...
          className="flex h-10 w-10 items-center justify-center border border-[#E2E8F0] text-[#0F172A] transition-colors hover:border-[#D4A72C] hover:text-[#D4A72C] lg:hidden"
        >
          ...
          <div className="flex flex-col">
            {links.map((link, index) => (
              <Link
                ...
                href={link.href}
                onClick={() => setMobileOpen(false)}
                className={`flex items-center justify-between py-4 text-xs font-bold uppercase tracking-[0.14em] text-[#334155] transition-colors hover:text-[#D4A72C] ${
